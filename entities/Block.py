from entities.GeometryObject import GeometryObject
...
class Block(GeometryObject):
    ...
    def drawToCanvas(self):
        ...
        if self.canvas.x1 <= self.x <= self.canvas.x2 and self.canvas.y1 <= self.y <= self.canvas.y2 and self.canvas.canvasArray[self.y - 1][self.x - 1] == ' ':
            ...
            b1 = Block(self.x, self.y + 1, self.penStyle)
            b1.drawToCanvas()
            b2 = Block(self.x, self.y - 1, self.penStyle)
            b2.drawToCanvas()
            b3 = Block(self.x + 1, self.y, self.penStyle)
            b3.drawToCanvas()
            b4 = Block(self.x - 1, self.y, self.penStyle)
            b4.drawToCanvas()
